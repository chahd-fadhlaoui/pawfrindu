...
const CategoriesCards = () => {  
  const categories = [  
    { category: "Dog", icon: dog, description: "Find loyal companions" },  
    { category: "Cat", icon: cat, description: "Discover feline friends" },  
    { category: "Other", icon: other, description: "Unique pets await"},  
    { category: "Veterinaries", icon: veterinary, description: "Expert pet care" },  
  ];  
  ...
  return (  
    <div className="relative px-4 py-16 overflow-hidden bg-white">  
      ...
      <div className="flex flex-wrap justify-center gap-6">   
        {categories.map((petcategory, index) => (  
          <Link   
            key={index}   
            to={`/pets/${petcategory.category}`} // Use Link to navigate  
            className={`  
          ...
          >  
            <div className="mb-4 p-4 bg-pink-50 rounded-full   
              ...
              <img  
                src={petcategory.icon}  
                alt={`${petcategory.category} icon`}  
