...
const AppointmentModal = ({ vet, onClose, onSuccess }) => {
  ...
  const isDayOpen = (date) => {
    ...
    return vet.veterinarianDetails?.openingHours?.[dayOfWeek] !== "Closed";
  };
  ...
  const getDaysInMonthView = () => {
    ...
    for (let i = firstDayOfWeek - 1; i >= 0; i--) {
      ...
      result.push({ date, isCurrentMonth: false, isOpen: isDayOpen(date) });
    }

    for (let i = 1; i <= lastDayOfMonth.getDate(); i++) {
      const date = new Date(year, month, i);
      result.push({ date, isCurrentMonth: true, isOpen: isDayOpen(date) });
    }
  ...
  const getAvailableTimeSlots = (date) => {
    ...
    return slots.filter((slot) => !isTimeSlotReserved(slot));
  };
  ...
  const isTimeSlotReserved = (time) => reservedSlots.includes(time);
  ...
  useEffect(() => {
    ...
    fetchReservedSlots();
  }, [selectedDate, vet._id]);

  useEffect(() => {
    if (selectedDate) setAvailableSlots(getAvailableTimeSlots(selectedDate));
  ...
  const handlePetSelect = (e) => {
    ...
    if (value === "other") {
      setSelectedPet(null); // Clear selectedPet for custom entry
    } else {
      ...
      return (
        <div className="py-8 flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-center text-gray-600 text-sm">
            Your appointment with Dr. {vet.fullName} for {selectedPet?.name || formData.customPetName} has been confirmed for {selectedDate?.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })} at {formatTimeSlot(selectedTime)}.
          </p>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${!day.isOpen || isPast ? "opacity-50 cursor-not-allowed" : "hover:bg-yellow-100"}
                      `}
                      onClick={() => !isPast && day.isOpen && handleDateSelect(day.date)}
                      disabled={isPast || !day.isOpen}
                      aria-label={day.date.toLocaleDateString()}
                      aria-selected={isSelected}
                      aria-disabled={isPast || !day.isOpen}
                    >
                      {day.date.getDate()}
                    </button>
                  );
                ...
                <div className="grid grid-cols-3 gap-2">
                  {availableSlots.map((time) => (
                    <button
                      ...
                      aria-label={`Select time: ${formatTimeSlot(time)}`}
