...
const LostPetUpdateForm = ({ reportId, onSuccess, onCancel }) => {
  const { user } = useApp();
  ...
  console.log(`Rendering LostPetUpdateForm for report ${reportId}`);
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        console.log(`Fetching report ${reportId} for LostPetUpdateForm`);
        const response = await axiosInstance.get(`/api/lost-and-found/${reportId}`);
        const report = response.data.data;
        if (!report) throw new Error("Report data not found");
        if (report.type !== "Lost") {
          console.error(`Report ${reportId} is not a Lost report, type: ${report.type}`);
          throw new Error("This form is for updating Lost reports only.");
        }
        console.log(`Fetched Lost report ${reportId}:`, { type: report.type });
        const reportDate = report.date ? new Date(report.date) : null;
        const newFormData = {
          type: "Lost",
          name: report.name || "",
          species: report.species ? report.species.toLowerCase() : "",
          breed: report.breed || "",
          size: report.size ? report.size.toLowerCase() : "",
          age: report.age || "",
          gender: report.gender || "",
          isPregnant: report.isPregnant ?? null,
          color: Array.isArray(report.color) ? report.color.map((c) => c.toLowerCase()) : [],
          location: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(`Initiating update for Lost report ${reportId}`);
    setSubmitting(true);
    setError(null);
    ...
    try {
      ...
      const report = response.data.data;
      if (report.type !== "Lost") {
        console.error(`Cannot update report ${reportId}: expected Lost, got ${report.type}`);
        setError("This form is for updating Lost reports only.");
        setSubmitting(false);
        return;
      }
      console.log(`Verified Lost report ${reportId} before update:`, { type: report.type });
    } catch (err) {
    ...
    const updateUrl = `/api/lost-and-found/${reportId}/update-lost`;
