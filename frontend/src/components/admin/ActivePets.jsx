...
const ActivePets = ({ onPetChange }) => {
  ...
  const cityOptions = [
    ...
    ...uniqueCities.map((city) => ({ value: city, label: city.charAt(0).toUpperCase() + city.slice(1) })),
  ];
  ...
  useEffect(() => {
    const applyFilters = () => {
      let filtered = pets.filter((pet) => !pet.isArchived); // Only active pets
    ...
    try {
      ...
      if (response.data.success) {
        setFilteredPets((prevPets) =>
          prevPets.map((pet) => (pet._id === petId ? { ...pet, status: "accepted" } : pet))
        );
        triggerPetsRefresh();
        onPetChange();
      } else {
  ...
  const handleArchive = async (petId) => {
    try {
      ...
      if (response.data.success) {
        setFilteredPets((prevPets) => prevPets.filter((pet) => pet._id !== petId)); // Remove from active list
        triggerPetsRefresh();
        onPetChange();
      } else {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setFilteredPets((prevPets) =>
          prevPets.map((pet) => (pet._id === selectedPet._id ? { ...pet, ...editFormData } : pet))
        );
        triggerPetsRefresh();
        onPetChange();
        setEditMode(false);
  ...
  const PetDetailsModal = ({ pet, onClose, actionLoading }) => {
    ...
    const getCandidateStatusIcon = (status) => {
      switch (status) {
        ...
        case "pending":
        default:
          return <Clock1 className="w-4 h-4 mr-1 text-orange-500" />;
      }
    };

    const approvedCandidate =
      pet.status === "adopted" && candidates.length > 0
        ...
        <div className="relative w-full max-w-lg p-6 bg-white border border-gray-200 shadow-xl rounded-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex flex-col items-center space-y-6">
            ...
            <div className="w-full p-4 border border-gray-200 bg-gray-50 rounded-xl">
              <h4 className="mb-4 text-lg font-semibold text-gray-800">Pet Details</h4>
              <div className="grid grid-cols-2 text-gray-700 gap-x-6 gap-y-3">
                ...
                <p className="col-span-2">
                  ...
                  <span className="ml-2 text-[#ffc929]">{pet.status}</span>
                </p>
              </div>
              <div className="mt-4">
                <p className="font-semibold text-gray-900">Description:</p>
                <p className="text-sm text-gray-600">{pet.description || "No description available."}</p>
              </div>
            </div>
            ...
            <div className="w-full p-4 bg-white border border-gray-200 shadow-sm rounded-xl">
              ...
              {fetchingCandidates ? (
                ...
                <p className="py-2 text-sm text-red-600">{candidatesError}</p>
              ) : candidates.length === 0 ? (
                <p className="py-2 text-sm italic text-gray-500">
                  {pet.status === "adopted" ? "No approved candidate found." : "No candidates have applied yet."}
                </p>
              ) : pet.status === "adopted" ? (
                <div className="space-y-4 text-gray-700">
                  ...
                  <p className="flex items-center gap-2">
                    ...
                    {approvedCandidate ? (
                      <span className="font-medium text-green-600">{approvedCandidate.name || "Unknown"}</span>
                    ) : (
                      <span className="italic text-gray-500">No approved candidate found</span>
                    )}
                  </p>
                </div>
              ) : (
                <ul className="space-y-3">
                  {candidates.map((candidate) => (
                    <li
                      key={candidate.id}
                      className="flex items-center justify-between p-3 border border-gray-100 rounded-md shadow-sm bg-gray-50"
                    >
      ...
      <>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <SearchBar
            ...
            placeholder="Search active pets by name, breed, city, or species..."
        ...
        {filteredPets.length === 0 ? (
          <div className="flex items-center justify-center p-12 bg-white border border-gray-200 shadow-sm rounded-xl">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-base font-medium text-gray-600">No active pets found</p>
          ...
          <div className="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-gray-800">Active Pet Listings</h2>
              </div>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-200">
                  {currentPets.map((pet) => {
                    ...
                    const canAccept = pet.status === "pending" && !pet.isArchived;
                    const canDelete = pet.status === "accepted" || isOwnPet;
                    const canArchive = (pet.status === "adopted" || pet.status === "sold") && !pet.isArchived;
                    const canEdit = isOwnPet && (pet.status === "pending" || pet.status === "accepted");
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 rounded-xl">
      ...
      {renderContent()}
      {selectedPet && <PetDetailsModal pet={selectedPet} onClose={() => setSelectedPet(null)} actionLoading={actionLoading} />}
